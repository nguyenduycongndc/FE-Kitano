...
namespace KitanoApplication.Controllers
{
    ...
    {
        public RolesController(IConfiguration config, ITokenService tokenService, ILogger<RolesController> logger) : base(config, tokenService)
        {
        }
        ...
        {
            if (Utils.IsCheckPemission("M_R", "PER_VIEW") == true)
            {
                return View();
            }
            else
